...
class Neuron(Module):
  def __init__(self, num_inputs: float, non_linear=True):
    self.weights: list[Value] = [Value(random.uniform(-1, 1)) for i in range(num_inputs)]
  ...
  def __call__(self, x: Value | float) -> Value:
    ...
    return z.relu() if self.non_linear else z
...
class Layer(Module):
  def __init__(self, num_inputs: float, num_outputs: float, **kwargs):
    self.neurons: list[Neuron] = [Neuron(num_inputs, **kwargs) for i in range(num_outputs)]
